  ...
  return (
    <div className="min-h-screen bg-app-bg text-primary flex flex-col font-sans selection:bg-blue-600/30 selection:text-white relative overflow-x-hidden">
      ...
      <div className="max-w-[1100px] w-full mx-auto px-6 py-12 md:py-20 flex-1 flex flex-col md:flex-row gap-12 relative items-start">
        ...
        <aside className="w-full md:w-[220px] shrink-0 sticky top-28 hidden md:block select-none" aria-label="Documentation sections">
          <div className="space-y-6">
            <div>
              <span className="text-[10px] font-mono text-neutral-500 font-semibold tracking-wider uppercase block mb-3">Product Docs</span>
        ...
        <main className="flex-1 space-y-24 md:space-y-32 max-w-[800px] w-full min-w-0">
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="space-y-4 text-sm sm:text-[15px] text-secondary leading-relaxed font-medium">
              ...
              <p>
                Instead of manually choosing between GPT, Claude, Gemini, or other providers, users simply ask their question and RouteMind handles the decision-making process. The goal is to simplify AI usage while improving quality, transparency, and efficiency.
              </p>
            </motion.div>
          </motion.section>
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="space-y-4 text-sm sm:text-[15px] text-secondary leading-relaxed font-medium">
              <p>
                Different AI models excel at different tasks. For instance, OpenAI's GPT models may perform better for coding, Anthropic's Claude may excel at creative writing, Google's Gemini excels at long document contexts, and custom Search Models are best for real-time information retrieval.
              </p>
              <p>
                Currently, users must manually decide which model to use before writing a prompt. This workflow creates:
              </p>
            </motion.div>

            <motion.ul variants={fadeInUp} className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs text-neutral-300 font-mono select-none">
            ...
            <motion.p variants={fadeInUp} className="text-xs text-neutral-500 leading-normal pt-2">
              RouteMind removes this burden by serving as an intelligent middleware layer that intercepts prompt intent and routes dynamically.
            </motion.p>
          </motion.section>
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="grid grid-cols-1 sm:grid-cols-5 gap-3 bg-card-bg border border-border-app p-4 rounded-xl text-center select-none font-mono text-[10px]">
              {[
                { step: '01', title: 'Query' },
            ...
            <motion.div variants={fadeInUp} className="space-y-4 font-mono text-xs text-neutral-400 pt-2">
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="p-5 bg-card-bg border border-border-app rounded-xl space-y-4 select-none font-mono text-[10px] text-center">
              <div className="grid grid-cols-1 sm:grid-cols-7 gap-2 items-center">
                <div className="p-2 bg-sidebar-bg border border-border-app rounded font-semibold text-neutral-400">User</div>
                <div className="text-neutral-600 hidden sm:block">──&gt;</div>
                <div className="p-2 bg-blue-950/20 border border-blue-500/20 text-blue-400 rounded font-semibold">Proxy Hub</div>
                <div className="text-neutral-600 hidden sm:block">──&gt;</div>
                <div className="p-2 bg-sidebar-bg border border-border-app rounded font-semibold text-neutral-400">Inference APIs</div>
                <div className="text-neutral-600 hidden sm:block">──&gt;</div>
                <div className="p-2 bg-green-950/20 border border-green-500/20 text-green-400 rounded font-semibold">Explain Layer</div>
              </div>
            </motion.div>

            <motion.div variants={fadeInUp} className="space-y-4 text-sm sm:text-[15px] text-secondary leading-relaxed font-medium">
              <p>
                When a query is entered in the Chat Workspace, it hits the **RouteMind proxy**. The intent classifier maps the structure and routes to the best API provider (Google, Anthropic, or OpenAI). The response aggregator wraps the raw token outputs and feeds metadata straight to the **Explainability Layer** before returning it to the user.
              </p>
            </motion.div>
          </motion.section>
          ...
          >
            ...
            <motion.ul variants={fadeInUp} className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs text-neutral-300 font-mono select-none">
              {[
                ...
                <li key={idx} className="flex items-center justify-between bg-card-bg border border-border-app p-3 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Check size={11} className="text-blue-400" />
                    <span>{item.title}</span>
                  </div>
                  <span className="text-[8px] px-1.5 py-0.5 rounded bg-blue-950/40 border border-blue-500/20 text-blue-400 font-semibold uppercase">{item.status}</span>
                </li>
              ))}
            </motion.ul>
          </motion.section>
          ...
          >
            ...
            <div className="space-y-3">
              ...
              >
                The future is not a single AI model. The future is intelligent orchestration of many models working together. RouteMind aims to become the operating system that makes this possible.
              </motion.p>
            </div>
